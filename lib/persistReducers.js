...
function persistReducer(config, baseReducer) {
  ...
  var conditionalUpdate = function conditionalUpdate(state) {
    ...
    state.has('_persist') && state.get('_persist').get('rehydrated') && _persistoid && !_paused && _persistoid.update(state);
  ...
  return function (state, action) {
    ...
    var handlesMap = (_handlesMap = {}, _defineProperty(_handlesMap, _reduxPersist.PERSIST, function () {
      var _sealed = false;
    ...
    }), _defineProperty(_handlesMap, _reduxPersist.PURGE, function () {
      _purge = true;
      action.result((0, _purgeStoredState2.default)(config));
      return baseReducer(restState, action).set('_persist', _persist);
    }), _defineProperty(_handlesMap, _reduxPersist.FLUSH, function () {
      action.result(_persistoid && _persistoid.flush());
      return baseReducer(restState, action).set('_persist', _persist);
    }), _defineProperty(_handlesMap, _reduxPersist.REHYDRATE, function () {
      if (_purge) {
      ...
      var newState = baseReducer(reconciledRest, action);
