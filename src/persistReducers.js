...
const hardSet = (payload, state) => state.merge(payload);
...
export default function persistReducer(config, baseReducer) {
  ...
  return (state, action) => {
    let _persist = state && state.has('_persist') && state.get('_persist') || null;
      ...
      if (action.key === config.key) {
        ...
        let reconciledRest =
          stateReconciler !== false && inboundState !== undefined
            ? stateReconciler(inboundState, reducedState)
            : reducedState;
        _persist = _persist && _persist.set('rehydrated', true) || Map({ rehydrated: true });
        let newState = baseReducer(reconciledRest, action).set('_persist', _persist);
